...
// Accept BOTH LeaderMath JWT (signed with JWT_SECRET) AND Firebase ID tokens.
// Returns { ok:true, loginId } or { ok:false, error: json(...) }
async function resolveLoginId(event){
  const token = getBearer(event);
  if(!token) return { ok:false, error: json(401, { error:"Token yo‘q" }) };

  // 1) Try LeaderMath JWT
  try{
    const payload = verifyToken(token);
    if(payload?.sub) return { ok:true, loginId: String(payload.sub), tokenType:"jwt" };
  }catch(_){}

  // 2) Try Firebase ID token
  try{
    // ensure firebase-admin app exists (getDb initializes admin)
    const decoded = await admin.auth().verifyIdToken(token);
    const loginId = decoded?.uid || decoded?.email;
    if(loginId) return { ok:true, loginId: String(loginId), tokenType:"firebase" };
  }catch(_){}

  return { ok:false, error: json(401, { error:"Token yaroqsiz" }) };
}
...
export const handler = async (event) => {
  try{
    ...
    if(path === "/results/submit" && method === "POST"){
      const auth = await resolveLoginId(event);
      if(!auth.ok) return auth.error;
