...
async function loadResults(){
  ...
  const qy = query(col, orderBy('createdAtFS','desc'), limit(20));
...
function renderBadges(arr){
  const w = qs('#badgesWrap'); w.innerHTML='';
  if(!arr || !arr.length){ w.innerHTML = '<div class="hint">Hozircha yutuqlar yo‘q.</div>'; return; }
  arr.forEach(b=>{
    const s=document.createElement('span'); s.className='pill'; s.textContent = b; w.appendChild(s);
  });
}
...
qs('#promoApply').addEventListener('click', async ()=>{
  ...
  if(!code){ msg.textContent='Kod kiriting'; return; }
  try{
    ...
    await runTransaction(db, async (tx)=>{
      ...
      if(D.expiresAt && D.expiresAt.toDate() < new Date()) throw new Error('Muddati tugagan');
...
async function adminListTop(){
  ...
  const users = await getDocs(query(collection(db,'users'), orderBy('numericId','asc'), limit(50)));
