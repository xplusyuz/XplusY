...
// NOTE: Fill your firebaseConfig in js/firebase-config.json (or inline below) before deploy.
...
function paintHeader(){
  ...
  if (!state.user){
    chipId.textContent = "#———";
    chipBal.textContent = "0 so‘m";
    chipGem.textContent = "0 💎";
...
async function ensureUserDoc(){
  ...
  if (!snap.exists()){
    // Allocate numericId atomically from meta/app
...
export async function redeemPromo(code){
  ...
  const codeRef = doc(state.db, 'promo_codes', code);
  const userRef = doc(state.db, 'users', state.user.uid);
  await runTransaction(state.db, async (tx)=>{
    const cs = await tx.get(codeRef);
    if (!cs.exists()) throw new Error('Kod topilmadi');
    const cd = cs.data();
    if (cd.expiresAt && cd.expiresAt.toDate && cd.expiresAt.toDate() < new Date()) throw new Error('Kod muddati tugagan');
    if ((cd.usesRemaining??0) <= 0) throw new Error('Kod ishlatilgan');
    const us = await tx.get(userRef);
    if (!us.exists()) throw new Error('User topilmadi');
    const addGems = cd.gems||0, addBal = cd.balance||0;
    tx.update(userRef, { gems: increment(addGems), balance: increment(addBal), lastPromoAt: serverTimestamp() });
...
export async function createTopUpRequest({amount, method, file}){
  ...
  const recRef = doc(state.db, 'topups', id);
  await setDoc(recRef, {
    uid: state.user.uid,
    amount, method, receiptUrl,
    status: 'pending',
    createdAt: serverTimestamp(),
  });
  return id;
...
export async function approveTopup(id){
  if (!state.userDoc) throw new Error('No admin');
  const isAdmin = state.userDoc.numericId==1000001 || state.userDoc.numericId==1000002 || state.userDoc.numericId=="1000001" || state.userDoc.numericId=="1000002";
  if (!isAdmin) throw new Error('Admin emas');
  const recRef = doc(state.db,'topups', id);
  const recSnap = await getDoc(recRef);
  if (!recSnap.exists()) throw new Error('Topup topilmadi');
  const { uid, amount } = recSnap.data();
  const userRef = doc(state.db, 'users', uid);
  await runTransaction(state.db, async (tx)=>{
