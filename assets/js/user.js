// Render header area and menu greeting based on auth state
...
async function renderAfterAuth(user = auth.currentUser){
  ...
  if(!user){
    el.innerHTML = `
      <button class="btn ghost" id="btn-login">Kirish</button>
      <button class="btn" id="btn-register">Registr</button>
    `;
  ...
  // Header: ID (with unvon svg), Balance (+), and small menu
  el.innerHTML = `
    <div class="tag" title="Foydalanuvchi ID">
      ${badgeSVG()} <strong>${u.numericId || "—"}</strong>
    </div>
    <a class="tag" href="balans.html" title="Balans">
      💳 <strong>${(u.balance||0).toLocaleString("uz-UZ")} so‘m</strong>
      <span class="btn" style="padding:4px 8px;border-radius:8px;margin-left:6px;">+</span>
    </a>
    <a class="btn ghost" href="profil.html">Profil</a>
  `;

  // Menu greeting with name + points + unvon
  if(greet){
...
function badgeSVG(){
  return `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path d="M12 2l3 6 6 .9-4.5 4.2 1 6L12 16l-5.5 3.9 1-6L3 8.9 9 8l3-6z" stroke="currentColor" stroke-width="1.5" fill="none"/>
  </svg>`;
}
...
// Example balance updater (call from payments flow)
