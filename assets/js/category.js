// Renders category pages as modern grid cards with SVG icons
async function includePartialsCategory(){
  ...
  // Wire header interactions again
...
async function renderCategory(){
  ...
  let data = [];
  try{
    data = await fetch(`assets/data/${cat}.json`).then(r=>r.json());
  }catch(e){
    // minimal fallback entries
    data = [
      { id: `${cat}-1`, title: "Boshlang‘ich 1", desc: "Namuna ta’lim birligi", icon:"general" },
      { id: `${cat}-2`, title: "Boshlang‘ich 2", desc: "Namuna ta’lim birligi", icon:"general" },
      { id: `${cat}-3`, title: "Boshlang‘ich 3", desc: "Namuna ta’lim birligi", icon:"general" },
    ];
  }
  grid.innerHTML = data.map(item => `
    <article class="card">
      ...
      <div class="row">
        <a class="btn" href="${item.href||'#'}">Kirish</a>
