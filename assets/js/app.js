// Partials, header wiring, olympiad grid, balance demo, and auth gate
...
function cardTemplate(o){
  ...
  const date = isNaN(d) ? o.when : d.toLocaleDateString("uz-UZ", {year:"numeric", month:"short", day:"2-digit"});
...
// Auth gate: show by default; hide after auth ready & logged in
function ensureAuthGate(){
  const required = document.body.dataset.requireAuth !== "false";
  if(!required) return;
  const gate = document.createElement("div");
  gate.className = "auth-gate"; gate.style.display = "grid";
...
window.ensureAuthGate = ensureAuthGate;
