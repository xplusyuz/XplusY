...
async function loadRoute(){
  ...
  try{
    ...
    app.innerHTML = `<div class="card" style="padding:16px; border:1px solid rgba(255,255,255,.06); border-radius:14px; background:var(--card); box-shadow: var(--shadow);">
      ...
      <p class="muted">Partial (partials/${route}.html) hali yaratilmagan.</p>
    </div>`;
  }
}
...
async function ensureUserNumericId(uid,email,name){
  ...
  const id=await runTransaction(db, async tx=>{
    ...
    if(m.exists() && m.data().nextNumericId){
      const raw=m.data().nextNumericId; next=typeof raw==="number"?raw:parseInt(raw,10)||next;
    }
...
async function getEmailByNumericId(numericId){
  ...
  const { email } = s.data();
  if(!email) throw new Error("Ushbu ID uchun email bog‘lanmagan (parol o‘rnatilmagan).");
  return email;
}
...
function renderAuthChipData(d){
  ...
  const gems=d?.gems??0;
  authChip.innerHTML = `
    <div class="id-badge" title="Profil ma'lumotlari">
      <span class="pill">
...
if (signupEmailForm) signupEmailForm.addEventListener("submit", async e=>{
  ...
  try{
    ...
    msg.textContent=`Tayyor! Sizning ID: ${numericId}. Endi kirishda faqat ID + parol.`; msg.classList.add("ok");
...
if (phoneSetPassForm) phoneSetPassForm.addEventListener("submit", async e=>{
  ...
  try{
    ...
    phoneMsg.textContent="Parol o‘rnatildi. Endi kirishda faqat ID + Parol foydalansiz."; phoneMsg.classList.add("ok");
    closeModal(authModal);
  }catch(err){
