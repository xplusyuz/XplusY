...
function renderAuthChipData(d){
  ...
  authChip.innerHTML=`<div class="id-badge" title="Profil">
    <span class="pill"><svg class="icon-12" viewBox="0 0 24 24"><path d="M4 4h16v16H4z" opacity=".15"/><path d="M7 8h10v2H7zM7 12h6v2H7z"/></svg><b>ID:</b> ${id}</span>
    <span class="sep"></span>
    <span class="pill"><svg class="icon-12" viewBox="0 0 24 24"><path d="M3 7h18v10H3z" opacity=".15"/><path d="M4 9h16v2H4zM6 13h8v2H6z"/></svg>Balans: ${balance.toLocaleString()}</span>
    <span class="pill"><svg class="icon-12" viewBox="0 0 24 24"><path d="M12 2l4 7-4 13-4-13z" opacity=".15"/></svg>Olmos: ${gems}</span>
  </div>`;
}
...
async function renderSignedIn(user){
  try{
    ...
    window.__unsubUser=onSnapshot(uref, us=>{
      ...
      const id=d.numericId||"—", balance=d.balance??0, gems=d.gems??0;
      if(panelUser){
        panelUser.innerHTML=`<div><b>ID:</b> ${id}</div><div class="muted">Balans: ${balance.toLocaleString()} • Olmos: ${gems}</div>
