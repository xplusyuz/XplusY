...
const fmt  = (x) => {
  ...
  return (Math.abs(r) % 1 === 0) ? String(Math.trunc(r)) : String(r);
};
...
const TEX_TOKENS = [
  ...
  "\\le","\\ge","\\cdot","\\times","\\pm","\\ln","\\log","\\sin","\\cos","\\tan","^","_"
];
...
const mjReady = () =>
  new Promise((res) => {
    const tick = () =>
      window.MathJax && window.MathJax.typesetPromise ? res() : setTimeout(tick, 30);
    tick();
  });
...
function applySectionRanges(data) {
  ...
  if (Array.isArray(data.sections)) {
    data.sections.forEach((sec) => {
      ...
      for (let i = s - 1; i < e; i++) {
        data.questions[i].section = data.questions[i].section || name;
      }
    });
  }
...
function refreshDotsState() {
  [...document.querySelectorAll(".dot")].forEach((el) => {
    ...
    const hasAns = !!(answers[i] && answers[i].length);
...
async function resolveImageSrc(idx1) {
  ...
  const cand = [
    `./${idx1}.jpg`, `./${idx1}.png`, `./${idx1}.jpeg`, `./${idx1}.webp`,
    q.image || ""
  ].filter(Boolean);
  for (const u of cand) {
...
function startTimerIfNeeded() {
  ...
  const both = (txt, show) => {
    $("#metaTimer").textContent = txt;
    $("#qTimer").textContent = txt;
    $("#metaTimer").classList.toggle("hidden", !show);
    $("#qTimer").classList.toggle("hidden", !show);
  };
...
function computeTotals() {
  ...
  testData.questions.forEach((q, i) => {
    ...
    const a = sectionAgg.get(sect);
    a.q += 1; a.ptsMax += pts; if (ok) { a.ok += 1; a.pts += pts; }
...
/* ========= Result + Users.points: single transaction ========= */
/**
 * Agar shu testCode uchun result hali yo'q bo'lsa:
 *  - results/{testCode}/users/{uid} ni yaratadi
 *  - users/{uid}.points ni ballga oshiradi
 * Aks holda (mavjud bo'lsa) hech narsa qilmaydi
 */
async function saveResultAndAddUserPointsOnce({ testCode, uid, total, max, detail, meta }) {
  const resRef  = doc(db, "results", testCode, "users", uid);
  const userRef = doc(db, "users", uid);

  return runTransaction(db, async (tx) => {
    const resSnap = await tx.get(resRef);
    if (resSnap.exists()) {
      return { skipped: true, already: resSnap.data() };
    }

    const userSnap = await tx.get(userRef);
    const prevPts  = userSnap.exists() ? Number(userSnap.data()?.points || 0) : 0;
    const add      = Number(total || 0);
    const nextPts  = prevPts + add;
    ...
    tx.set(resRef, {
      ...
      title:  (meta?.title  || ""),
      ball: Number(add),
      firstTotal: Number(add),
      max: Number(max || 0),
      percent: (max ? Math.round((add / max) * 100) : 0),
      detail: Array.isArray(detail) ? detail : [],
      createdAt: serverTimestamp(),
      spentSeconds: Number(meta?.spentSeconds || 0)
    });
    ...
    return { skipped: false, added: add, totalPoints: nextPts };
  });
}
...
async function onFinish() {
  ...
  if (startedAt) {
    const now = new Date();
    spentSeconds = Math.max(spentSeconds, Math.floor((now - startedAt) / 1000));
  }
  ...
  const safeDetail = perQuestion.map((r) => ({
    i: r.i,
    section: r.section || "Umumiy",
    pts: Number(r.pts || 0),
    ok: !!r.ok,
    picked: Array.isArray(r.picked) ? r.picked : [],
    correct: Array.isArray(r.correct) ? r.correct : [],
    text: String(r.text || ""),
    options: Array.isArray(r.options) ? r.options.map((v) => String(v ?? "")) : []
  }));
  ...
  try {
    const meta = { testId: (testData.id || currentTestId), title: (testData.title || ""), spentSeconds };
    const res = await saveResultAndAddUserPointsOnce({
    ...
    if (res.skipped) {
      showSaveStatus("Oldin yechilgan — points qo‘shilmadi, result o‘zgarmadi", "warn");
    } else {
      showSaveStatus(`Saqlandi (ball=${total}) · Users.points +${res.added} → ${res.totalPoints}`, "good");
    }
  } catch (e) {
...
async function boot() {
  ...
  // Test bor → boshlang'ich ko'rinish
