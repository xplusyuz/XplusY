...
exports.claimAdmin = onCall({ region: "us-central1", cors: true }, async (req) => {
  ...
  // Configure admin emails (allowlist)
// Option A (recommended): set env var in Cloud Functions runtime
//   ORZU_ADMIN_EMAILS="a@b.com,c@d.com"
// Option B (legacy Firebase functions:config):
//   firebase functions:config:set orzu.admin_emails="a@b.com,c@d.com"
let raw = (process.env.ORZU_ADMIN_EMAILS || "").trim();
try {
  const legacy = require("firebase-functions").config?.() || {};
  if (!raw && legacy?.orzu?.admin_emails) raw = String(legacy.orzu.admin_emails).trim();
} catch (e) { /* ignore */ }
