const { db, collection, query, where, orderBy, getDocs, doc, getDoc, requireAuth } = window.fb;
...
requireAuth(()=>{
  const qp = new URLSearchParams(location.search).get('testId');
  if(qp){ input.value=qp; loadRatingForTest(qp); }
});
...
exportBtn.addEventListener('click', ()=>{ const csv=toCSV(tableEl); const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='test-reyting.csv'; a.click(); URL.revokeObjectURL(url); });
loadBtn.addEventListener('click', ()=>{ const id = input.value.trim(); if(!id) return; history.replaceState(null,'',`?testId=${encodeURIComponent(id)}`); loadRatingForTest(id); });
